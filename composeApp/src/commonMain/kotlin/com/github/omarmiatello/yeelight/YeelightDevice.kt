        ...
        aSocket(YeelightManager.ioSelector).tcp()
            ...
            .use { socket ->
                socket.openWriteChannel(autoFlush = true).writeStringUtf8("${cmd.realCommand}\r\n")
                    .also { if (enableLog) logger.info { "$id --> ${cmd.realCommand}" } }
                try {
                    socket.openReadChannel().readUTF8Line()
                        .also { if (enableLog) logger.info { "$id <-- $it" } }
                } catch (e: SocketTimeoutException) {
                    if (enableLog) logger.error { "$id <-- !! Response Timeout !! (request: ${cmd.realCommand})" }
                    null
                }
            }
    }
